<template>
  <div class="h-[100vh] bg-[#fdfbfc] pb-[12vw]">
    ...
    <img src="/public/logo.fill.svg" alt="" class="w-[38vw] mx-auto mt-[7vw] mb-[9vw]" />
    <!-- 二维码 -->
    <img
      :src="qrimgs"
      alt=""
...
const getKey = async () => {
  try {
    const res = await axios.get("https://netease-serrver01.vercel.app/login/qr/key", {
      params: {
...
const loginQqr = async (key) => {
  try {
    const res = await axios.get("https://netease-serrver01.vercel.app/login/qr/create", {
      params: {
        timerstamp: new Date().getTime(),
        qrimg: true,
        key: key,
...
const qrCheck = async () => {
  try {
    const res = await axios.get("https://netease-serrver01.vercel.app/login/qr/check", {
      params: {
        key: unikey.value,
        timerstamp: new Date().getTime(),
...
const getStatus = async () => {
  try {
    const res = await axios.get(
      `https://netease-serrver01.vercel.app/login/status?cookie=${localforage.getItem("cookie")}`
    );
...
const getUinfo = async () => {
  try {
    const res = await axios.get(`https://netease-serrver01.vercel.app/user/detail?uid=32953014`);
